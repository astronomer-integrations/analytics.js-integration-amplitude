...
var Amplitude = (module.exports = integration('Amplitude')
  ...
  .option('usePageDetailsForTrackCalls', false)
...
Amplitude.prototype.page = function(page) {
  ...
  // if categorized or named tracking is true (UI Custom Tracking checked), build event name based on category and/or name
  if (opts.trackCategorizedPages || opts.trackNamedPages) {
    var names = [];
    if (category && opts.trackCategorizedPages) names.push(category);
    if (name && opts.trackNamedPages) names.push(name);
    this.track(page.track(names.join(' ')));
  } else if (opts.trackAllPages) { // if trackAllPages is true (UI Default Tracking checked)
...
function logEvent(track, dontSetRevenue) {
  ...
  if (this.options.usePageDetailsForTrackCalls) {
    var pageDetails = track.proxy('context.page');
    props = Object.assign(props, pageDetails);
  }
